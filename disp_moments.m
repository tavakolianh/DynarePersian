...
if options_.nomoments == 0
    ...
    title='گشتاورهای متغیرهای شبیه‌سازی شده';

    title=add_filter_subtitle(title,options_);

    headers=char('\text{متغیر}','\text{میانگین}','\text{انحراف معیار}','\text{واریانس}','\text{چولگی}', ...
                 '\text{کشیدگی}');
...
if options_.nocorr == 0
    ...
    if options_.noprint == 0
        title = 'همبستگی متغیرهای شبیه‌سازی شده';

        title=add_filter_subtitle(title,options_);

        headers = char('\text{متغیر}',M_.endo_names(ivar,:));
        dyntable(options_,title,headers,labels,corr,size(labels,2)+2,8,4);
        if options_.TeX
            headers = char('\text{متغیر}',M_.endo_names_tex(ivar,:));
            lh = size(labels,2)+2;
...
if ar > 0
    ...
    if options_.noprint == 0
        title = 'خودهمبستگی متغیرهای شبیه‌سازی شده';
        title=add_filter_subtitle(title,options_);
        headers = char('\text{متغیر}', int2str([1:ar]'));
...
if ~options_.nodecomposition
    if M_.exo_nbr == 1
        ...
        if ~options_.noprint %options_.nomoments == 0
            skipline()
            title='تجزیه واریانس شبیه‌سازی یک تکانه در هر لحظه (به درصد)';
