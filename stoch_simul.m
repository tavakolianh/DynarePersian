function info=stoch_simul_(var_list)
...
if ~options_.noprint
    ...
    disp('خلاصه مدل')
    skipline()
    disp(['  تعداد متغیرها:         ' int2str(M_.endo_nbr)])
    disp(['  تعداد تکانه‌های تصادفی: ' int2str(M_.exo_nbr)])
    disp(['  تعداد متغیرهای وضعیت:   ' int2str(M_.nspred)])
    disp(['  تعداد متغیرهای جهشی:           ' int2str(M_.nsfwrd)])
    disp(['  تعداد متغیرهای ایستا:  ' int2str(M_.nstatic)])
    my_title='ماتریس کوواریانس تکانه‌های برون‌زا';
    labels = deblank(M_.exo_names);
    headers = char('\text{متغیرها}',labels);
    lh = size(labels,2)+2;
